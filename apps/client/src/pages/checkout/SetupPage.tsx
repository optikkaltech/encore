...
export default function SetupPage() {
  ...
  useEffect(() => {
    if (!token || orderRefParam) return; // skip if we're in a callback flow
  ...
  const handleCompleteSetup = async () => {
    ...
    try {
      ...
      sessionStorage.setItem('setup_payment_method', paymentMethod);
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg-secondary)', padding: '24px 16px' }}>
        <div className="card" style={{ padding: 48, maxWidth: 460, textAlign: 'center', animation: 'slideUp 300ms ease-out' }}>
          {isAlreadyUsed ? (
            <>
              <CheckCircleIcon size={52} style={{ color: 'var(--success)', margin: '0 auto 20px', display: 'block' }} />
              ...
              {savedMerchantId && (
                ...
                >
                  <LogInIcon size={16} /> Go to Portal Login
                </button>
              )}
            </>
          ) : (
            <>
              <AlertCircleIcon size={52} style={{ color: 'var(--warning)', margin: '0 auto 20px', display: 'block' }} />
            ...
            <div>
              ...
              <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: '14px 16px', marginBottom: 28 }}>
                <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <MailIcon size={16} style={{ color: 'var(--text-tertiary)', flexShrink: 0 }} />
                  <span>Verified: <strong>{info?.subscriber.email}</strong></span>
                </p>
              </div>
          ...
          {(step === 'payment' || step === 'completing') && (
            <div>
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid var(--border-primary)', marginBottom: 20, gap: 0 }}>
                {(['card', 'direct_debit'] as const).map(m => (
                  <button key={m} onClick={() => setPaymentMethod(m)} style={{ flex: 1, padding: '8px 4px', fontSize: 12, fontWeight: paymentMethod === m ? 700 : 400, color: paymentMethod === m ? brand : 'var(--text-secondary)', background: 'none', border: 'none', borderBottom: paymentMethod === m ? `2px solid ${brand}` : '2px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                    {m === 'card' ? <CreditCardIcon size={14} /> : <LandmarkIcon size={14} />}
                    <span>{m === 'card' ? 'Card' : 'Bank'}</span>
                  </button>
                ))}
              </div>
              {paymentMethod === 'card' && (
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 16, marginBottom: 24, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <LockIcon size={16} style={{ color: 'var(--text-secondary)', flexShrink: 0, marginTop: 2 }} />
              ...
              {paymentMethod === 'direct_debit' && (
                <div style={{ background: 'var(--bg-secondary)', borderRadius: 8, padding: 16, marginBottom: 24, display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  <LandmarkIcon size={16} style={{ color: 'var(--text-secondary)', flexShrink: 0, marginTop: 2 }} />
                  <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, margin: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={handleCompleteSetup} disabled={step === 'completing'} style={{ flex: 2, padding: '12px 0', borderRadius: 8, border: 'none', background: brand, color: '#fff', fontWeight: 700, cursor: step === 'completing' ? 'wait' : 'pointer', opacity: step === 'completing' ? 0.7 : 1 }}>
                  {step === 'completing' ? 'Setting up...' : `Connect ${paymentMethod === 'card' ? 'Card' : 'Bank'} via Nomba →`}
                </button>
              </div>
            </div>
          )}
